...
export function ProducerConfiguracion() {
  return (
    <div className="flex flex-col gap-6">

      <div>
        <h1 className="font-display text-2xl font-bold text-gray-900">Configuración</h1>
        <p className="text-sm text-gray-400 mt-0.5">Preferencias de cuenta, seguridad y notificaciones</p>
      </div>
      ...
      <Section icon={Bell} title="Notificaciones" description="Elige cuándo y cómo te notificamos">
        <div className="space-y-3">
          {[
            { label: 'Nueva cotización recibida',    sub: 'Cuando alguien solicita precio de tus cosechas',    on: true  },
            { label: 'Lead nuevo',                   sub: 'Cuando alguien muestra interés en tu producción',   on: true  },
            { label: 'Cosecha próxima a agotarse',   sub: 'Alerta cuando el volumen disponible es bajo',       on: true  },
            { label: 'Nueva reseña',                 sub: 'Cuando un comprador deja una calificación',         on: false },
            { label: 'Documento por expirar',        sub: 'Alerta 30 días antes de vencimiento de documentos', on: true  },
      ...
      <Section icon={Lock} title="Seguridad" description="Contraseña y acceso a la cuenta">
        <div className="space-y-3">
          ...
          <button disabled className="flex items-center gap-2 rounded-xl bg-agrobot-600 px-4 py-2 text-sm font-bold text-white opacity-50 cursor-not-allowed">
            <Save className="h-4 w-4" /> Cambiar contraseña (próximamente)
          </button>
        </div>
      </Section>
      ...
      <div className="rounded-xl border border-red-200 bg-red-50/50 p-5">
        ...
        <p className="text-xs text-red-600 mb-4">Eliminar tu cuenta es permanente e irreversible. Perderás todas tus publicaciones, inventario y documentos.</p>
        <button disabled className="flex items-center gap-2 rounded-xl border border-red-300 px-4 py-2 text-sm font-bold text-red-500 opacity-50 cursor-not-allowed">
          <Trash2 className="h-4 w-4" /> Eliminar cuenta (próximamente)
        </button>
      </div>
